...
class Utils:
    ...
    def do_login(self):
        try:
            res = requests.post(f"https://your-api.com/api/v1/login", json={"username":self.config["verwaltung"]["admin"]["username"], "password":self.config["verwaltung"]["admin"]["password"]}, headers={"Content-Type":"application/json"}).content
    ...
    def find_video_in_list_by_id(self, videos, id):
        for video in videos:
